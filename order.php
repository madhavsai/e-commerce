...
<link rel="stylesheet" type="text/css" media="screen and (min-width:801px) and (max-width:2440px)" href="profile.css">
 <title>90cart</title>
</head>
<body onload="totalprice();" >
  ...
  <div class="head">
      ...
      <div class="delivery_con" id="delcont"><form method="POST" onSubmit="return login_db()" action="javascript:return login_db();">
        Delivary Services<i class="fa fa-remove" style="font-size:24px;margin-left:80px;cursor:pointer;" onclick="del_ordercls();"></i>
  ...
  <div class="content">
          ...
          <div class="cart_content">
            	<div class="cart_container">
          				...
          				<div class="cart_details"><div class="item_detailss"><span class="dddetails">Product Details</span></div><div class="quantityy"><span class="dddetails">Price</span></div><div class="price_detailss"><span class="dddetails">Address</span></div><div class="delivery_detailss"><span class="dddetails">productname</span></div></div>
                  <?php
                    ...
                    $uni=$_SESSION['uid'];
                    $res_sel =mysql_query("select * from `$uni`");
                    while($fet_sel =mysql_fetch_array($res_sel))
                   { ?>
          					<div class="cart_proo">
          						<div class="item_detailss">         <span class="uuudetails">         <?php   include "products_db.php";
                                        $newvar=$fet_sel['products_ids'];
                                        $newvar1=$fet_sel['quantity'];
                                        $newval=explode(",",$newvar);
                                        $newval1=explode(",",$newvar1);
                                        $len=count($newval);
                                       $x=0;
                                        for($x =0; $x <= $len-1; $x++)
                                        {
                                          $varb=$newval[$x];
                                          $qnt=$newval1[$x];
                                          $res_sel2=mysql_query("select * from `product` where product_id='$varb'");
                                          while($fet_sel1 =mysql_fetch_array($res_sel2))
                                          {
                                            $varb1=$fet_sel1['name'];
                                            echo "$varb1($qnt) <br>";
                                          }
                                        ...
                                        ?></span></div>
                                        <div class="quantityy">
          				 <span class="uuudetails">Rs:  <?php echo $fet_sel['totalprice'];?></span></div>
          				<div class="price_detailss"> <span class="uuudetails">  <?php echo $fet_sel['address'];?></span></div>
          				<div class="delivery_detailss"> <span class="uuudetails">  <?php echo $fet_sel['productsname'];?></span></div>
          	  			</div>
          			<?php	}
          ?>
          	</div>
